...
func buildBrowserPageSection() []string {
	return []string{
		...
		"",
		"CRITICAL — do not loop on refresh_page_content:",
		"- One snapshot is enough. After a successful refresh_page_content, pick a selector from the list and call execute_action immediately. Do NOT call refresh_page_content twice in a row — the snapshot will be identical and you will waste the user's turn.",
		...
		"Count the items the user listed and match that exact count of fill+press_enter pairs before the final reply. Stopping early is a failure mode.",
		"",
