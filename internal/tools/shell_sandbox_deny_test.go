...
func TestRelaxSandboxDenyGroups(t *testing.T) {
	...
	if got["reverse_shell"] {
		t.Error("reverse_shell should be relaxed (false) for sandboxed exec")
	}
	if got["package_install"] {
		t.Error("package_install should be relaxed (false) for sandboxed exec")
	}
	...
	// but must still block a non-relaxed group (destructive_ops: rm -rf).
	pats := ResolveDenyPatterns(got)
	for _, blocked := range []string{
		`python3 -c "import requests"`,
		`pip install requests`,
	} {
		if matchesAny(blocked, pats) {
			t.Errorf("sandbox-relaxed policy still blocks %q", blocked)
		}
	}
