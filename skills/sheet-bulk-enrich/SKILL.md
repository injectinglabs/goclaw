...
- **Spawn ALL N subagents in ONE assistant turn.** Issue every `spawn` call (one per row) in a single message so the runtime fans them out maximally in parallel — do NOT batch or throttle. Then call `spawn({action:"wait"})` once to collect them all. Sequential or batched spawning serialises the wall-clock and is slower.
...
### Step 4 — Spawn ALL N subagents in ONE turn, each writing its OWN row live

Issue every `spawn` call (one per row, N total) in a SINGLE assistant turn so the runtime fans them all out in parallel at once. Do NOT batch, throttle, or spread them across turns — maximum parallelism is the point. Then go to Step 5 (one `wait`).

**Concurrency is parallel.** The runtime dispatches all N spawn calls simultaneously; wall-clock ≈ the slowest single subagent (~30-60 s with web_search), not the sum across rows.

**Each subagent does ONE `web_search`, then commits its row directly via `BULK_SHEET_WRITE`.** No web_fetch (Wikipedia returns 20-50KB of text which blows up context and slows things 4×). If `web_search` returns nothing useful, the subagent MUST fall back to training knowledge and still write the row — empty cells are ONLY for fields the subagent genuinely can't recall AND search didn't cover.
...
**Web-search note**: Tavily is configured on stage, so `web_search` returns real results. If a query still comes back empty, the "fallback to training" rule above keeps the row filled.

**Expected per-subagent cost: ~3-8 K tokens (one LLM turn + one web_search), ~5-15 seconds** depending on search provider speed. With all N spawned at once, total wall-clock ≈ the slowest single subagent (~30-60 s), not the sum across rows.

### Step 5 — Wait once for all N subagents

After spawning all N in the previous turn, issue ONE tool call — `spawn` with action `wait`:

```json
{ "tool": "spawn", "arguments": { "action": "wait", "timeout": 600 } }
```

`wait` blocks until every child completes (or `timeout` seconds elapse). It returns a formatted list, one line per task, with the task's full result text (capped at 4 KB per task). One `wait` collects the whole fan-out.
...
If any task shows `[failed]`: include the row in the final summary but DON'T fail the run. That row's cells will simply remain empty.
