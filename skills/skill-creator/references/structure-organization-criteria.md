...
Skills are created directly in `~/skills-store/<skill-name>/`.
After writing SKILL.md, use `publish_skill` to register in the DB (version managed by agent runtime).
...
### scripts/
Executable code for deterministic tasks. Runs via the runtime's `exec` tool (no sudo needed).
Packages installed at runtime persist to `/app/data/.runtime/`.
...
```bash
python3 scripts/skill-creator/scripts/package_skill.py ~/skills-store/<name>
```
