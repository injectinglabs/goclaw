...
- agent runtime uses directory name as the DB slug — `name` is also used as display name fallback
...
agent runtime reads only `name` and `description` from frontmatter (the `Metadata` struct).
