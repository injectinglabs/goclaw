...
## Runtime Environment (agent runtime)

Scripts run inside the the agent runtime container via the `exec` tool. Environment is set by the entrypoint:

| Variable | Value | Purpose |
|----------|-------|---------|
| `PYTHONPATH` | `/app/data/.runtime/pip` | Python runtime packages |
| `PIP_TARGET` | `/app/data/.runtime/pip` | pip install target |
| `NPM_CONFIG_PREFIX` | `/app/data/.runtime/npm-global` | npm global install dir |
