	...
	return func(job *store.CronJob) (*store.CronJobResult, error) {
		...
		if job.Deliver && job.DeliverChannel != "" && job.DeliverTo != "" {
			...
			if channels.IsInternalChannel(job.DeliverChannel) {
				// Broadcast the result to the tenant's WS clients so the
				// extension / CLI can inject it into their UI. Clients fall
				// back to the active chat if they can't match session_key.
				slog.Info("cron: broadcasting delivery to internal channel",
