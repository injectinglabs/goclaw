...
func builtinToolSeedData() []store.BuiltinToolDef {
	defs := []store.BuiltinToolDef{
		...
		{Name: "batch_web_search", DisplayName: "Batch Web Search", Description: "Run many web searches concurrently in one call (one query per item) — for building a sheet/table of N items without separate searches or sub-agents", Category: "web", Enabled: true,
			Metadata: json.RawMessage(`{"config_hint":"Config → Tools → Web Search"}`),
		},
		{Name: "research_sheet", DisplayName: "Research Sheet", Description: "Build and deliver a real web-researched spreadsheet (.xlsx) for a list of items — searches + extracts each item's pages, pulls emails/website + columns from live page text, then writes and delivers the file", Category: "web", Enabled: true,
			Metadata: json.RawMessage(`{"config_hint":"Config → Tools → Web Search"}`),
		},
