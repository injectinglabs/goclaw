...
func builtinToolSeedData() []store.BuiltinToolDef {
	defs := []store.BuiltinToolDef{
		...
		{Name: "research_sheet", DisplayName: "Research Sheet", Description: "Build real, web-researched data rows for a list of items (searches each item + extracts columns) — the data engine for a researched spreadsheet, instead of recalling values from memory", Category: "web", Enabled: true,
			Metadata: json.RawMessage(`{"config_hint":"Config → Tools → Web Search"}`),
		},
